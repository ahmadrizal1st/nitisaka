...
export function GlowyWavesHero({ children, className = "" }: { children?: React.ReactNode, className?: string }) {
    ...
    const computeThemeColors = () => {
      ...
      const resolveColor = (variables: string[], alpha = 1) => {
        ...
        for (const variable of variables) {
          ...
          if (value) {
            tempEl.style.backgroundColor = `hsl(var(${variable}))`; // Shadcn uses HSL variables
            let computedColor = getComputedStyle(tempEl).backgroundColor;

            // Fallback if HSL doesn't work, try direct var
  ...
  return (
    <div
      className={`relative isolate flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-background ${className}`}
      role="region"
